...
`voice-router v1` 现在已经不是概念稿，而是做完并实测过的版本。
...
所以这版不是“设计中”，而是已经进入 **working v1** 状态。
...
经过真实测试，当前 token plan 稳定支持：

- `speech-2.8-hd`

因此 v1 里 MiniMax 的稳定默认模型固定为：

- `model = speech-2.8-hd`

这样 provider 链路更简单，也更稳定。
